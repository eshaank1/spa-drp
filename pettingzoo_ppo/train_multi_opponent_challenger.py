...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a challenger against a target and evaluate against multiple opponents (previous gen, original PPO, SmartBot, RandomBot)."
    )
    parser.add_argument("--challenger", type=str, required=True, help="Path to the challenger .zip to continue training.")
    parser.add_argument("--target-opponent", type=str, required=True, help="Path to the primary training opponent (usually previous gen).")
    ...
    # Build training environment where opponent is the primary target (previous champion)
    ...
    for it in range(1, args.max_iters + 1):
        print(f"\nContinue-iter {it}: training {args.timesteps_per_iter} steps against primary opponent...")
        ...
        opponents = [
            ("Target (Previous Gen)", args.target_opponent),
            ("Original PPO", args.original_ppo),
            ("SmartBot", None),  # Special marker for SmartBot
            ("RandomBot", None),  # Special marker for RandomBot
        ]
        ...
        for opp_name, opp_path in opponents:
            ...
            if opp_name == "SmartBot":
                # For SmartBot, use SmartBot opponent in env
                smartbot_env = CardGameVsSmartParallelEnv(
                    seed=args.seed + it,
                    invalid_action_penalty=1.0,
                    opponent_model_path=None,  # No model, use default SmartBot
                ...
                # For RandomBot, use RandomBot opponent in env
                randombot_env = CardGameVsSmartParallelEnv(
                    seed=args.seed + it + 100,
                    invalid_action_penalty=1.0,
                    opponent_model_path=None,
                    opponent_deterministic=False,  # RandomBot is non-deterministic
                )
                wins, losses, ties = 0, 0, 0
                for ep in range(args.eval_episodes):
                ...
                # For PPO opponents, use evaluate_model_vs_opponent
