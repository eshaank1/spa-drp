...
class MultiOpponentWrapper:
    """Wrapper that rotates through multiple opponents during training."""
    def __init__(self, num_envs: int, seed: int, opponent_paths: list[Optional[str]]):
        self.num_envs = num_envs
        self.seed = seed
        self.opponent_paths = opponent_paths  # Can contain None for SmartBot/RandomBot
        self.envs = []
        self.current_opponent_idx = 0
        self._build_envs()

    def _build_envs(self):
        opponent = self.opponent_paths[self.current_opponent_idx]
        env = CardGameVsSmartParallelEnv(
            seed=self.seed,
            invalid_action_penalty=1.0,
            opponent_model_path=opponent,
            opponent_deterministic=True if opponent else True,
        )
        env = ss.pettingzoo_env_to_vec_env_v1(env)
        env = ss.concat_vec_envs_v1(env, self.num_envs, num_cpus=0, base_class="stable_baselines3")
        env = VecMonitor(env)
        self.envs = [env]
    ...
    def rotate_opponent(self):
        """Switch to next opponent in the pool."""
        self.current_opponent_idx = (self.current_opponent_idx + 1) % len(self.opponent_paths)
        self.envs[0].close()
        self._build_envs()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a challenger against all 4 opponents (previous gen, original PPO, SmartBot, RandomBot) simultaneously."
    )
    ...
    # Pool of opponents (None = use SmartBot/RandomBot)
    opponents = [
        args.previous_gen,     # PPO: previous gen
        args.original_ppo,     # PPO: original
        None,                  # SmartBot
        None,                  # RandomBot (handled by opponent_deterministic=False internally)
    ]
    ...
    env = CardGameVsSmartParallelEnv(
        seed=args.seed,
        invalid_action_penalty=1.0,
        opponent_model_path=opponents[0],
    ...
    for it in range(1, args.max_iters + 1):
        print(f"\nContinue-iter {it}: training {args.timesteps_per_iter} steps against opponent pool...")
        
        # Train for full timesteps, rotating opponents periodically
        steps_per_opponent = args.timesteps_per_iter // len(opponents)
        for opp_idx in range(len(opponents)):
            current_opponent_idx = opp_idx
            opp_name = ["Previous Gen", "Original PPO", "SmartBot", "RandomBot"][opp_idx]
            
            print(f"  Training {steps_per_opponent} steps vs {opp_name}...")
            
            # Rebuild env for this opponent
            env.close()
            env = CardGameVsSmartParallelEnv(
                seed=args.seed + it + opp_idx,
                invalid_action_penalty=1.0,
                opponent_model_path=opponents[opp_idx],
                opponent_deterministic=True,
            )
            env = ss.pettingzoo_env_to_vec_env_v1(env)
            env = ss.concat_vec_envs_v1(env, args.num_envs, num_cpus=0, base_class="stable_baselines3")
            ...
            model.learn(total_timesteps=steps_per_opponent, callback=checkpoint_cb, reset_num_timesteps=False)
        ...
        for opp_name, opp_path in opponents_eval:
            ...
            if opp_name == "SmartBot":
                smartbot_env = CardGameVsSmartParallelEnv(
                    seed=args.seed + it,
                    invalid_action_penalty=1.0,
                ...
                for ep in range(args.eval_episodes):
                    obs, _ = smartbot_env.reset()
                    final_info = None
                    while smartbot_env.agents:
                        action, _ = model.predict(obs["learner"], deterministic=True)
                        obs, reward, term, trunc, info = smartbot_env.step({"learner": int(action)})
                        final_info = info.get("learner", final_info)
                        if term.get("learner", False) or trunc.get("learner", False):
                            break
            ...
            elif opp_name == "RandomBot":
                randombot_env = CardGameVsSmartParallelEnv(
                    seed=args.seed + it + 100,
                    invalid_action_penalty=1.0,
                    opponent_model_path=None,
                ...
                wins, losses, ties = 0, 0, 0
                for ep in range(args.eval_episodes):
                    obs, _ = randombot_env.reset()
                    final_info = None
                    while randombot_env.agents:
                        action, _ = model.predict(obs["learner"], deterministic=True)
                        obs, reward, term, trunc, info = randombot_env.step({"learner": int(action)})
                        final_info = info.get("learner", final_info)
                        if term.get("learner", False) or trunc.get("learner", False):
                            break
        ...
        print("\n✗ Not all thresholds passed. Continuing training...")
